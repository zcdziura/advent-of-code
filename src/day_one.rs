...
static PART_ONE_INPUT: [&str; 1000] = [
    "four9one",
...
fn run_part_one() {
    let total: usize = PART_ONE_INPUT
        .iter()
        .map(|input| {
            let (first, second) = input
                ...
                .rev()
                .last()
                .zip(input.chars().rev().filter(|c| c.is_numeric()).rev().last())
                .unwrap();
    ...
    println!("DAY ONE, PART ONE: {total}");
}

static PART_TWO_INPUT: [&str; 1000] = [
    "four9one",
    "bbzhsmnmtf8kftwosevenxfkssgrcjthree",
    "6pkkcddsixsixjgnjvdtjtwo",
    "4four45seven7nine7two",
...
fn run_part_two() {
    let digits = HashSet::<&str>::from_iter(["1", "2", "3", "4", "5", "6", "7", "8", "9"]);
    let digit_mapping = HashMap::<&str, &str>::from_iter([
        ("one", "1"),
        ("two", "2"),
        ("three", "3"),
        ("four", "4"),
    ...
    let total: usize = PART_TWO_INPUT
        .iter()
        .map(|input| {
            let mut found_numbers = NUMBERS
                .iter()
                .flat_map(|number| {
                    input.match_indices(number).map(|(idx, _)| {
                        if digits.contains(number) {
                            (idx, *number)
                        } else {
                            let num = digit_mapping.get(number).unwrap();
                            (idx, *num)
                        }
                    })
                })
            ...
            found_numbers.sort_by(|(a, _), (b, _)| a.partial_cmp(b).unwrap());

            let (first, second) = found_numbers
                .iter()
                .map(|(_, num)| num)
                .rev()
                .last()
                .zip(found_numbers.iter().rev().map(|(_, num)| num).rev().last())
                .unwrap();
    ...
    println!("DAY ONE, PART TWO: {total}");
}
